...
def test_resolve_golden_clone_source_maps_default_image_to_dotfiles_golden() -> None:
...
def test_resolve_golden_clone_source_uses_a_forward_looking_convention_for_other_images() -> None:
    assert _resolve_golden_clone_source("tahoe") == "dotfiles-golden-tahoe"


def test_resolve_golden_clone_source_rejects_an_undeclared_image() -> None:
    try:
...
def test_diff_command_shell_quotes_a_mount_point_containing_spaces() -> None:
...
def test_diff_command_is_a_no_op_quote_for_a_path_with_no_spaces() -> None:
    assert _diff_command("/tmp/dotfiles") == "chezmoi diff --source=/tmp/dotfiles"


def test_destroy_impl_stops_before_deleting(fake_process: FakeProcess) -> None:
...
def test_run_impl_forwards_dotfiles_to_up_impl(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_up_impl(*, vm: str, image: str, dotfiles: str | None = None) -> dict[str, Any]:
        captured["dotfiles"] = dotfiles
        return {"vm": vm, "run_id": "20260710-000000-000000", "ip": "10.0.0.5"}

    def fake_apply_impl(*, vm: str, version_manager: str) -> dict[str, Any]:
        return {"ok": True}
