...
from macos_ci._doctor_core import CheckResult, DoctorFacts, check, overall_ok, version_at_least

GOOD_FACTS: DoctorFacts = DoctorFacts(
    tool_versions={
        "tart": "2.32.1",
        "packer": "1.15.4",
        "just": "1.42.4",
...
def test_check_reports_version_below_minimum() -> None:
...
def test_check_fails_on_non_apple_silicon() -> None:
...
def test_check_fails_on_locked_login_keychain() -> None:
...
def test_check_fails_when_zsh_dotfiles_missing() -> None:
...
def test_check_fails_on_low_disk_space() -> None:
...
def test_check_always_reports_fleet_ceiling_never_gates_it() -> None:
    results = check(GOOD_FACTS)
...
def test_version_at_least_treats_a_short_form_as_equal_when_numerically_equal() -> None:
...
def test_version_at_least_still_rejects_a_genuinely_lower_short_form() -> None:
    assert version_at_least("1.9", "2.0.0") is False


def test_version_at_least_baseline_equal_length_still_passes() -> None:
    assert version_at_least("2.0.0", "2.0.0") is True


def test_check_requires_sshpass_for_the_ssh_bootstrap_phase() -> None:
...
def test_check_passes_when_sshpass_is_present() -> None:
