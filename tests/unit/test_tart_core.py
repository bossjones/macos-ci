...
def test_run_argv_headless_default() -> None:
    assert run_argv("dotfiles-test") == ["tart", "run", "dotfiles-test", "--no-graphics"]


def test_run_argv_with_dir_mount() -> None:
    argv = run_argv(
        "dotfiles-test",
...
def test_run_argv_with_readonly_dir_mount() -> None:
    argv = run_argv(
        "dotfiles-test",
...
def test_run_argv_vnc_experimental() -> None:
...
def test_ip_argv_default_resolver() -> None:
    assert ip_argv("dotfiles-test") == ["tart", "ip", "dotfiles-test", "--resolver", "dhcp"]


def test_ip_argv_agent_resolver() -> None:
...
def test_tart_clone_invokes_no_real_tart_binary(fake_process: FakeProcess) -> None:
