...
def test_chezmoi_init_only_argv_has_no_apply_or_force_flags() -> None:
...
def test_chezmoi_argv_matches_smoke_test_docker_verbatim() -> None:
...
def test_chezmoi_argv_parameterized_on_version_manager() -> None:
    argv = chezmoi_argv(source=".", version_manager="asdf")
    assert "version_manager=asdf" in argv
    assert "version_manager=mise" not in argv


def test_chezmoi_argv_does_not_include_the_retry_wrapper() -> None:
...
def test_format_run_id_is_deterministic_given_explicit_inputs() -> None:
    assert format_run_id(timestamp="20260710-181234", suffix="ab12cd") == "20260710-181234-ab12cd"


def test_clone_name_matches_spec_08_naming() -> None:
...
def test_chezmoi_identity_env_sets_computer_name_and_hostname() -> None:
...
def test_artifact_paths_layout_matches_spec_12_contract() -> None:
...
def test_bootstrap_ssh_argv_uses_sshpass_and_password_auth() -> None:
...
def test_bootstrap_ssh_argv_parameterized_on_user_and_password() -> None:
...
def test_ssh_opts_carry_a_server_alive_backstop() -> None:
...
def test_steady_state_ssh_argv_is_batch_mode_yes_key_based() -> None:
...
def test_steady_state_ssh_argv_exports_toolchain_path_first() -> None:
...
def test_steady_state_ssh_argv_never_carries_a_tty_flag() -> None:
...
def test_steady_state_ssh_argv_exports_identity_env_after_path_before_the_command() -> None:
    argv = steady_state_ssh_argv(
