...
class CoverageSummary(TypedDict):
    """The ``summary`` section of ``pyrefly coverage report`` JSON output."""
...
def format_comparison_markdown(
    ...
    lines = [
        ...
        (
            ...
            f"| {pr['coverage']:.2f}% | {_fmt_delta(coverage_delta)}% |"
        ),
        (
        ...
        (
            ...
            f"| {pr['n_typed']:,} | {_fmt_delta(typed_delta, ',')} |"
        ),
        (
            f"| Untyped symbols | {base['n_untyped']:,} "
            f"| {pr['n_untyped']:,} | {_fmt_delta(untyped_delta, ',')} |"
        ),
        (
